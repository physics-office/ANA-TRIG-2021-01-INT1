...
\section{\textcolor{red}{The ATLAS Detector}}\label{sec:context}
...
\subsection{Calorimeter System}\label{ssec:calo}
...
% The inner detector, employed for charged-particle tracking, is immersed in a
% \SI{2}{\tesla} axial magnetic field produced by a thin superconducting solenoid
...
% interaction point~\cite{PERF-2015-08,inner_tdr1,inner_tdr2}: Pixel Detector (92
% million channels), Silicon Microstrip Detector (SCT, 6.3 million channels) and
% Transition Radiation Detector (TRT, 350 thousand channels). The TRT offers
% electron identification capability within its pseudorapidity coverage ($|\eta|
% \lesssim 2$) via the detection of transition-radiation photons generated by the
% radiators. Track reconstruction requires (pile-up dependent) CPU intensive
% computations due to the complexity of image-processing algorithms with
% combinatorial nature~\cite{PERF-2015-08}.  During 2016, it was the second
...
algorithm to provide its decision.%~\cite{ATLAS-PERF-2017-01-002}
...
performed by the online framework for the EGAM1 and EGAM7 derivations.%, and
