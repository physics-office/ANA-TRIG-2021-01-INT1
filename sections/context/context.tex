...
sections (Section~\ref{ssec:calo}), which \textcolor{red}{enveloped} an inner \textcolor{red}{tracking system}. The two-level triggering system reduces the total
...
\section{Inner Detector}\label{ssec:track}
...
variables for describing the shower shape in the EM2). The objective is to reduce the dimensionality of the problem and} compose a highly discriminant input space \textcolor{red}{to apply} a set of rectangular cuts, which provides a simple yet powerful selection mechanism. The \textcolor{use of} variables providing similar information
...
$\eta$, both the KDE and discriminant requirements are computed in delimited \textcolor{ref}{($\eta\times~E_{T})$}
