...
%To reduce implementation efforts, electron triggers executing the Neural\rnn{}
%information compute their information after the cut-based reconstruction, whose
%decision is not computed. As a result, the Neural\rnn{} electron 
...
2017.
%Thus, it allows to assess the direct impact of applying the Neural\rnn{} selection
...
%   mutually exclusive cases: both with and without Neural\rnn{} triggered (black); triggered only
%   with Neural\rnn{} (blue); triggered only without Neural\rnn{} (red); neither one triggered
...
% \reta{}, \rphi{} and \wstot{} where the Neural\rnn{} trigger is consistently
...
% \emi{}. The Neural\rnn{} trigger behavior shows even lower effect in the \rhad{},
...
% slight bias towards collecting more events in the signal region with the Neural\rnn{}
% trigger. Interestingly, some few events are accepted by the trigger with Neural\rnn{}
...
% Neural\rnn{} selection collects electrons with lower leakage in \emiii{} for region
% $2.01<\abseta{}<2.37$, whereas the opposite occurs in region
% $2.37<\abseta{}<2.37$. We believe this behavior to be associated with the usage of a single neural network during 2017 to cover the full end-cap region, but this could not be confirmed since 2018 data taking did not employ the duplicated trigger setup any longer.  As already mentioned, the $2.37<\abseta{}<2.37$ region is particularly different to the others due to the lack of the strips in EM1. Hence, the Ringer performance suffered deterioration at this region and, in order to keep the same signal efficiency, the operation point needed to be set looser, allowing the trigger to collect less signal-like electrons. This behavior is due to the usage of a single neural
...
% lack of the strips in \emi{}. Hence, Neural\rnn{} performance suffered deterioration
...
% both Neural\rnn{} and the cut-based triggers select the electrons in
% Figure~\ref{fig:quadrant_rhad_et4_eta4}.}, which is near the starting point of
% the second peak for the electrons accepted only by the Neural\rnn{} selection, which
...
%analysis, we observe how the usage of the different triggers (from Ringer and cut-based algorithms) may impact the likelihood algorithm. This 
...
%This study requires a pair of duplicated triggers (with and without the Neural\rnn{}) operating online together. We discuss the
...
%     Neural\rnn{} and cut-based algorithms were optimized on different datasets, but
...
%e28\_lhtight\_nod0\_ivarloose trigger, which operated with Neural\rnn{} algorithm in
%the \fastcalo{} step.
