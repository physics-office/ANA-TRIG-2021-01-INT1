\section{\text{High Level Trigger} Deployment}%
\label{sec:operation}
...
% As observed in the efficiency measurements during Run~2 data taking, the NeuralRinger
...
% requirements of the NeuralRinger in a very important primary electron trigger
% (Section~\ref{top:cpu_e26}).

% \subsubsection{\fastcalo{} CPU Demands}\label{top:fastcalo_cpu}

% To reduce implementation efforts, electron triggers executing the NeuralRinger
% information compute their information after the cut-based reconstruction, whose
% decision is not computed. As a result, the NeuralRinger electron triggers always
...
% compute the NeuralRinger decision by running a single electron trigger with and without
% the NeuralRinger algorithm in two individual non-concurrent executions using the same
...
% the \fastcalo step of electron triggers with (white) and without (yellow) NeuralRinger
...
% in the \fastcalo step of electron triggers with (white) and without (yellow) NeuralRinger
% using EB events. More details in description of Figure~\ref{fig:fastcalo_fex_time}. 
% The center histogram represents the CPU time to store the NeuralRinger output into the AOD 
% format (persistence).}
% \end{figure}
...
% persistent format. 
% Specifically, NeuralRinger hypothesis testing is considerably more demanding
...
% Taking into account the referred values, the NeuralRinger can require a
% relative increase of up to \SI{50}{\%} in the \fastcalo{} CPU time per event
% with respect to the trigger without NeuralRinger\footnote{We expect the result to be
% dependent on the trigger configuration due to presence of pile-up. Reported
% results are for e17\_lhvloose\_nod0.}. Nonetheless, the NeuralRinger contributes to
...
% avoiding the computation of fake candidates with the NeuralRinger algorithms using the
% lowest-energy-threshold unprescaled single electron trigger
% (e26\_lhtight\_nod0\_ivarloose). We executed it with and without the NeuralRinger
...
% 327265 processed by release AthenaP1,21.1.55. The NeuralRinger algorithm provided a
