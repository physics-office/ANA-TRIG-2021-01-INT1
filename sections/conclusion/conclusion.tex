...
%CPU demanding, thus motivating studies to evaluate whether additional \rnn{}
...
%However, combined photon triggers must be considered carefully. Specifically, if we the \rnn{} is set to
%operate only for very demanding combined photon triggers, the decision
%correlation on fakes for triggers with and without the \rnn{} will play an
...
%is to deploy a strategy similar to the \rnn{} in the hardware based selection.
...
%investigating the possibilities for the \rnn{} employment with the super-cells.
%It is worth mentioning that the operation of \rnn{} at the earlier stages can be
